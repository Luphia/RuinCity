 ...
 * 「離線也能玩」的支柱就垮了。
 *
 * ★ 2 → 3.6 是**地圖放大到 900×900 的伴隨改動**（等比 ×1.8），
 *   不是一次獨立的平衡調整：地圖幾何整組 ×1.8，速度也 ×1.8，
 *   於是**每一段行軍所需的真實時間完全不變** ——
 *   鄰居突襲仍是十幾分鐘、預警窗口不變、遠征仍需前哨營接力，
 *   `docs/11` §15 那一整組模擬結論因此仍然適用。
 *
 *   少了這一步，症狀是很具體的：出生帶外緣（離自家遺跡 189 格）的玩家
 *   帶攻城單位跑一趟要 8 小時以上，**打不到自己陣營的遺跡** ——
 *   而遺跡是整個賽季的目標（`docs/05`）。這是 map.test 照出來的。
 */
export const MARCH_SCALE = 3.6 as const;
...
export const SEASON_MODIFIERS: Record<Season, SeasonModifiers> = {
  ...
  /**
   * ★ production 0.55 → 0.70、upkeep 1.40 → 1.55。
   *
   *   舊組合（0.55 / 1.40）讓收支在入冬瞬間惡化 **3.05 倍**（1.2/0.55 × 1.4），
   *   賽季模擬顯示中位數玩家在入冬第一週就被餓掉一半的部隊 ——
   *   「用兵在秋冬兩季達高峰」因此退化成「只有秋季有高峰」。
   *
   *   0.70 / 1.55 的擺盪是 2.66 倍：軍隊還爬得進冬季，
   *   接著在月 11–12 被消耗回 1,950 上下（相對峰值折損約 20%）——
   *   冬天依然很痛，但不是清空。
   *
   *   upkeep 從 1.40 再往上是 -d 版的調整：天花板放寬之後整體經濟變富
   *   （耕作科技滿級就是 +48% 糧食），舊值下冬季只有 45% 的玩家
   *   收支轉負，壓力不夠。
   */
  WINTER: {
    label: "長夜",
