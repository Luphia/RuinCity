...
export interface RuinSpec {
   ...
   *   清空 < 15%」永遠不可能成立。
   *
   *   拉開到約 1.7 倍一階之後，三座各差一個月倒下（第 6 / 7 / 8 月），
   *   順序也對得上增益強度：行軍加成最弱 → 最先倒，攻防加成最強 → 最後倒。
   *
   *   上限同樣有講究：軍團以 ×1.15/月複利成長，而玩家軍隊在後期會頂到
   *   人口天花板。基礎值一旦拉太高，第三座遺跡在整個賽季內
   *   **永遠打不下來** —— 一個中途變成不可能的目標比太簡單更糟。
   *
   *   絕對值在 `-d` 版又整體下修了約 36%（8k/14k/22k → 5k/9k/14k）：
   *   天花板放寬之後玩家把資源分散到更多地方，遊戲月 5–6 的軍隊規模
   *   比 `-c` 版小了一半，舊門檻誰都打不動。
   */
