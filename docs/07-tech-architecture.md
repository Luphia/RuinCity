...
900×900 = 810,000 格。全量傳輸不可行。分層處理：
...
- L3 戰略視圖用另一個端點 `/api/map/overview`，
  回傳 900×900 每格 1 byte 的「聯盟 ID」點陣圖（810 KB → gzip 約 45 KB），
...
每一個需要玩家身分的路徑都要先回答同一個問題：
**這個帳號現在在哪一場賽季裡？**

在 M5c 之前，那個問題在十個地方各被寫了一次 SQL —— 據點、領土、軍事、
集市、執政官、HUD、地圖總覽、地圖個人圖層、單格戰場、出局判定。
十份實作也就有十組略微不同的條件，而它們**遲早會對同一位玩家給出
不同的答案**。這不是假設：地圖總覽用的是「最新的、還沒封存的那一場」，
於是每一場賽季走到第 7 天（下一場開放登記），所有還在打的人的地圖
就換成了另一個世界。

現在只有一份：`lib/server/current-player.ts`。

```ts
currentPlayer()        // 我的玩家（出局／放棄的也回傳）
currentLivePlayer()    // 還在場上的（「有據點可以走進去嗎」用這個）
requirePlayerId()      // 寫入路徑：拿不到就丟 UNAUTHENTICATED / NO_PLAYER
currentPlayerByEmail() // 已經有 email 的呼叫端（可注入 db，整合測試用）
```

判準：**這個帳號在未封存賽季裡的 `players` 列，取最新的一場。**

| 選擇 | 理由 |
| --- | --- |
| 從 `players` 出發，不從 `seasons` | 下一場第 7 天就開登記，而那時候我還在打這一場 |
| `ENDING` 也算 | 終戰期的賽季凍結但仍然是你的。只認 `RUNNING` 會讓最後兩天的每一頁都說「你還沒有進行中的賽季」 |
| 出局／放棄的人也回傳 | 他要看得到自己的世界與結束畫面。寫入的守門在 `settleWithin`（`PlayerEliminatedError`），不需要也不該在這裡擋 |

**這條規則寫成了測試**（`current-player.guard.test.ts`）：掃過 `app/` 與
`lib/`，只有 `current-player.ts` 與 `account.ts` 可以出現 `schema.users.email`。
靠讀碼守不住 —— 每一次新增都只是「再寫一次那個 join」，看起來完全無害。

### 地圖多一層

`/api/map/overview` 走同一個解析，另外兩條保險：

- **有賽季的人拿不到自己那一場的地形 → 回 404**，不退回 `s0` 開發地圖。
  把另一個世界畫給他看比報錯更糟：他會照著一張錯的地圖規劃一整晚的行軍。
  地形本來就補得回來（`ensureLatestTerrain` 以 seed 重新生成）。
- 回應帶著 `viewerSeason`，客戶端再對一次帳；不一致就出紅色橫幅。
  伺服器已經保證了，但這個錯誤的代價高到值得再擋一層。
