...
| 中立地的混戰 | 清完守衛之後**攻方彼此開打**（`docs/04` §3d）：依抵達順序接力，先到的當守方、後到的一個一個挑戰，站到最後的拿走那塊地。不要改成「每一方 vs 其餘合計」—— 三方兵力相近時會一輪全滅。有主的格子**不混戰**（攻方是同一邊的，五個名額就是為了湊拆塔傷害）|
| 地圖上的資源地 | 畫**地貌**（稻田／森林／山洞／礦坑，`lib/game/map-icon.ts`），荒地與山脈不畫；**等級用面積表示**（lv1 佔 38%、lv5 佔 92%）。圖示走**貼圖 + sprite 池**，不是一張大 Graphics —— 幾百個圖示塞進同一張 Graphics，即使不重建 Pixi 仍然每幀重新處理那幾千個圖元。貼圖依「資源 × 實際像素尺寸」快取（`scaleMode` 是 nearest，縮放會碎）。稻田的田面是**綠的**：藍色在地圖上已經是河與湖 |
| 資源圖示 | 畫面上的糧／木／石／鐵一律走 `components/ui/ResourceIcon`（12×12 像素，`lib/game/resource-icon.ts`）。名字**不在圖示模組裡定義** —— 規格是 `balance/economy.ts` 的 `RESOURCE_LABEL`。`<option>` 塞不了 SVG，那裡用全名 |
| 主城圖示 | 地圖上的主城佔滿 **2×2**（`lib/game/keep-icon.ts`），依**城牆**（RAMPART，不是主堡）分三段：木寨／石垣／鐵壁。三段的**輪廓與比例**要不同，只差色階等於沒有分段。旗是聯盟色，由渲染層代入。≥16px/格才畫圖示，更遠退回原本的色塊 |
| 交戰動畫 | 正在打的那一格畫**兩把互砍的刀**（`lib/game/war-icon.ts`），嚴格在 **1×1** 內，16px 以下不演（退回脈動紅 ✕）。它是裝飾，不讀任何戰鬥狀態 —— 與「進行中的動畫 losses 是空的」同一條規矩 |
| 動畫什麼時候收 | 下架時刻由 `engagements.ends_at` 決定，**不是**由輪詢決定：交戰只有兩分鐘而輪詢十五秒一次。而「現在還在打嗎」只有一份實作（`lib/render/battles.ts` 的 `isBattleLive`），場景與 footer 共用 —— 兩份會讓地圖還在砍、按鈕已經說打完了 |
| 烽火要跟 `SceneData` 分開 | 交戰十五秒換一次、`SceneData` 一場賽季換一次。併在一起的話，`MapCanvas` 的對焦 effect 會每十五秒把相機拉回據點（與 §20.23 同類） |
